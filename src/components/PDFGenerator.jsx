...
const PDFGenerator = ({ formData, customerData, calculations, formatCurrency, loggedBy, onSaved }) => {
  ...
  const generatePDF = async () => {
    // loggedBy is only required when this component is embedded in the
    // logged-in admin Leads page (LeadCalculatorPanel passes onSaved). The
    // public-facing PaymentCalculator doesn't pass onSaved at all, so we
    // only enforce the requirement when that prop is actually present.
    if (onSaved && !loggedBy) {
      alert('⚠ Please select which staff member is logging this estimate before generating.');
      return;
    }
