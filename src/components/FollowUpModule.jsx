...
const FollowUpModule = () => {
  ...
  const handleUpdateLead = async (id, updates) => {
    try {
      await supabase.from('follow_ups_2024').update({...updates, updated_at: new Date().toISOString()}).eq('id', id);
      fetchFollowUps();
  ...
  const handleAddRemark = async (e) => {
    ...
    try {
      ...
      setReminderDate('');
      fetchFollowUps();
      setSelectedItem(prev => ({...prev, history: updatedHistory, next_follow_up: reminderDate || prev.next_follow_up, next_action_type: actionType}));
    } catch (err) {
      alert(err.message);
    } finally {
      setIsSaving(false);
    ...
    <div className="flex flex-col xl:flex-row gap-6 h-[calc(100vh-180px)]">
      <div className="w-full xl:w-80 shrink-0 space-y-4">
        <div className="bg-white p-6 rounded-[32px] border shadow-sm border-gray-100">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <p className="text-[8px] font-black text-indigo-400 uppercase tracking-[0.2em] mt-1">Registry v2.4</p>
            </div>
          </div>
        ...
        <div className="bg-white p-4 rounded-[24px] border shadow-sm border-gray-100 relative">
          ...
          <input type="text" placeholder="Search Database..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-3 bg-gray-50 border-none rounded-xl text-[10px] font-black uppercase outline-none" />
        </div>
      </div>
      ...
      <div className="flex-1 bg-white rounded-[40px] border shadow-sm border-gray-100 overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4 no-scrollbar bg-gray-50/20">
          {filteredData.map(item => (
            <div key={item.id} onClick={() => setSelectedItem(item)} className={`p-6 rounded-[32px] border transition-all cursor-pointer relative overflow-hidden ${selectedItem?.id === item.id ? 'bg-indigo-600 text-white shadow-2xl border-indigo-600' : 'bg-white border-gray-100 hover:border-indigo-200'}`}>
        ...
        {selectedItem ? (
          <div className="bg-white h-full rounded-[40px] shadow-2xl border border-gray-100 overflow-hidden flex flex-col animate-in slide-in-from-right-8 duration-300">
            ...
            <div className="flex divide-x divide-gray-100 border-b border-gray-100 shrink-0 bg-white">
              <QuickActionBtn label={selectedItem.status === 'Closed' ? "RE-OPEN" : "CLOSE LOG"} icon={selectedItem.status === 'Closed' ? FiActivity : FiCheckSquare} onClick={() => handleUpdateLead(selectedItem.id, { status: selectedItem.status === 'Closed' ? 'Active' : 'Closed' })} color={selectedItem.status === 'Closed' ? 'text-green-600' : 'text-red-500'} />
              ...
              <QuickActionBtn label="DELETE" icon={FiTrash2} onClick={() => handleDelete(selectedItem.id)} color="text-red-400" />
            </div>
                  ...
                  <form onSubmit={handleAddRemark} className="space-y-4">
                    <div className="bg-white p-6 rounded-[32px] border border-gray-200 shadow-sm transition-all hover:border-indigo-100">
                      <div className="grid grid-cols-2 gap-4 mb-4">
                        <div className="space-y-1">
                          <label className="text-[8px] font-black text-gray-400 uppercase ml-1">Handled By</label>
                          <select value={selectedStaff} onChange={e => setSelectedStaff(e.target.value)} className="w-full px-4 py-2 bg-gray-50 border border-gray-100 rounded-xl text-[10px] font-black uppercase outline-none focus:ring-2 focus:ring-indigo-100">
                        ...
                        <div className="space-y-1">
                          ...
                          <select value={actionType} onChange={e => setActionType(e.target.value)} className="w-full px-4 py-2 bg-gray-50 border border-gray-100 rounded-xl text-[10px] font-black uppercase outline-none focus:ring-2 focus:ring-indigo-100">
                            {['Call','Visit','Payment','Document'].map(t => <option key={t} value={t}>{t}</option>)}
                          </select>
                        </div>
                      </div>
                      <textarea value={newComment} onChange={e => setNewComment(e.target.value)} placeholder="Interaction details..." rows={2} className="w-full px-5 py-4 bg-gray-50 border-none rounded-2xl text-[11px] font-bold outline-none placeholder:text-gray-300 focus:ring-4 focus:ring-indigo-100" />
                      <div className="grid grid-cols-2 gap-3 mt-4">
                        <input type="date" value={reminderDate} onChange={e => setReminderDate(e.target.value)} className="w-full px-4 py-3 bg-gray-50 border-none rounded-xl text-[9px] font-black outline-none" />
                        <button type="submit" disabled={isSaving || !newComment.trim()} className="bg-indigo-600 text-white rounded-xl font-black uppercase text-[10px] tracking-widest shadow-xl">Save Remark</button>
                      </div>
                    </div>
                  </form>
                </div>
