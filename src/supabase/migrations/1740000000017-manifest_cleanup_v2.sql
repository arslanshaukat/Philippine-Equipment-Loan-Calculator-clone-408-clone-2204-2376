/* 
# Final Shipment Manifest Cleanup
1. Removal
  - Drops the old `shipments_2024` flat table entirely to prevent data overlap.
  - Clears `shipment_headers_20240218` and `shipment_units_20240218` of all placeholders.
2. Data Restoration
  - Re-inserts ONLY the 5 specific records managed by the user.
*/

-- 1. DROP THE IRRELEVANT FLAT TABLE
DROP TABLE IF EXISTS shipments_2024 CASCADE;

-- 2. WIPE ACTIVE MANIFEST TABLES
TRUNCATE TABLE shipment_units_20240218 CASCADE;
TRUNCATE TABLE shipment_headers_20240218 CASCADE;

-- 3. INSERT USER RECORD 1: Jan 12, 2026 (PAID)
WITH h1 AS (
...
-- 4. INSERT USER RECORD 2: Jan 28, 2026 (UNPAID)
WITH h2 AS (
...
-- 5. INSERT USER RECORD 3: Feb 14, 2026 (PAID)
WITH h3 AS (
  INSERT INTO shipment_headers_20240218 (soa_date, bl_no, vessel_name, voyage_no, kgs, cbm, amount_php, is_paid, payment_date)
...
-- 6. INSERT USER RECORD 4: Feb 25, 2026 (UNPAID)
WITH h4 AS (
  INSERT INTO shipment_headers_20240218 (soa_date, bl_no, vessel_name, voyage_no, kgs, cbm, amount_php, is_paid)
...
-- 7. INSERT USER RECORD 5: Mar 05, 2026 (PAID)
WITH h5 AS (
  INSERT INTO shipment_headers_20240218 (soa_date, bl_no, vessel_name, voyage_no, kgs, cbm, amount_php, is_paid, payment_date)
